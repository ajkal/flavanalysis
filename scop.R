##
## R functions for dealing with SCOP data
##  Version 0.4   Thu Apr  5 19:02:26 EDT 2012  (update for v1.75)
##  Version 0.3   Tue Apr  1 15:32:29 PDT 2008  (update for v1.73)
##  Version 0.2   Wed Mar  5 16:17:29 PST 2008  (update for v1.71)
##  Version 0.1   Wed Aug 31 10:52:29 PDT 2005  (update for v1.69)
##
...
## To Update
##scop <- load.scop(T, inpath="http://scop.mrc-lmb.cam.ac.uk/scop/parse/", outpath="/Users/ajkal/Work/flavoproteins/", version=1.75)
...
load.scop <- function(flag2read = FALSE, hash=TRUE,
  ...
  if(!flag2read) {
    ...
    if(!file.exists(outpath)) {
      warning("Outpath requested does not existing, trying to create directory...")
      dir.create(outpath)
    }

    ## Check on inpath and read from online if it does not exist.
    if (substr(inpath, 1, 4) != "http") {
      if(!file.exists(inpath)) {
        warning("Inpath does not exist, trying to read from online source...")
        inpath="http://scop.mrc-lmb.cam.ac.uk/scop/parse/"
      }
    }
    ...
    ## 3. Comment file (not really necessary)
    ...
    ##- Create Annotation Object (a combination of des and cla files)
    cat("Creating new $anot annotation object: this can be slow\n")
...
seqaln.scop <- function(sids, outfile="raw_aln.fa", ...) {
  ...
  ##  results are written to "outfile" and put returned as a
  ##  data.frame
  ## Usage:
